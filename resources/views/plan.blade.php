        ...
        <section class="space-y-6 animate-[rise_0.8s_ease-out]">
            ...
            <div class="grid gap-6 lg:grid-cols-[1.25fr,0.9fr]">
                <div>
                    ...
                    <div
                        class="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/30 px-4 py-6"
                        x-show="snapshotModalOpen"
                        x-transition.opacity
                        @keydown.escape.window="closeSnapshotModal"
                    >
                        <div class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" @click.outside="closeSnapshotModal">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Snapshot note</p>
                                    <h3 class="mt-2 text-xl font-semibold text-slate-900">Add a quick context note</h3>
                                    <p class="mt-1 text-sm text-slate-500">Optional. Add a short note about why this snapshot matters.</p>
                                </div>
                                <button
                                    class="inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:text-slate-700"
                                    type="button"
                                    @click="closeSnapshotModal"
                                    aria-label="Close snapshot note"
                                >
                                    <x-heroicon-o-x-mark class="h-4 w-4" />
                                </button>
                            </div>
                            <textarea
                                class="mt-4 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-slate-400 focus:outline-none"
                                rows="5"
                                placeholder="Example: Switched jobs this month, so income may fluctuate."
                                x-model="snapshotNote"
                            ></textarea>
